...
function mapreducekernel(
    ...
    block_start = (bid - 0x01) * chunksize + 0x01
    block_end = min(block_start + chunksize - 0x01, n)

    block_start > n && return

    @inbounds if length(Vs) == 0x01
        i = block_start + tid - 0x01
        val = f(Vs[1][i])
        i += threads
        last = block_end - threads << 2
        while i <= last
            v1 = f(Vs[1][i])
            i += threads
            v2 = f(Vs[1][i])
    ...
    elseif length(Vs) == 2
        val = f(Vs[1][block_start+tid-0x01], Vs[2][block_start+tid-0x01])
        i = block_start + threads + tid - 0x01
        while i <= block_end
            val = op(val, f(Vs[1][i], Vs[2][i]))#
            i += threads
        end
    else
        val = f((V[block_start+tid-0x01] for V in Vs)...)# Somehow less optimized than two previous functions
        #i = block_start + threads + tid - 0x01
        for i in (block_start+threads+tid-0x01:threads:block_end)
    ...
    @inbounds if ((lane == warpsz && tid < block_end - block_start + 0x01) || (tid == block_end - block_start + 0x01))
        shmem_res[wid] = val
    end

    if threads <= block_end - block_start + 0x01
        wid != cld(threads, warpsz) && return
    else
        (wid != cld(block_end - block_start + 0x01, warpsz)) && return
    end
    ...
    filtr = (
        (threads <= block_end - block_start + 0x01 && lane == cld(threads, warpsz)) ||
        (threads > block_end - block_start + 0x01 && (lane == cld(block_end - block_start + 0x01, warpsz)))
    )
    @inbounds if filtr
        partial[bid] = val
        threadfence()
    ...
    bid % warpsz != 0x01 && return #only keep first warp of each block

    @inbounds if blocks == 0x01
        if filtr
            result[1] = val
        end
        return
