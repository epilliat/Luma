...
R = copy(V)
while time() - start_time < 0.500  # 500ms warm-up
    CUDA.@sync accumulate!(+, R, V)
end
prof = CUDA.@bprofile time = 0.05 accumulate!(+, R, V)
dt = 0
dts = []
while dt <= 2 * tmax_timed
    timed = (CUDA.@timed accumulate!(+, R, V))#(CUDA.@timed CUDA.@sync eval(exp_expr))[:time]
    u = timed[:time]
    dt += u
    push!(dts, u)
end
timed = (CUDA.@timed CUDA.@sync accumulate!(+, R, V))
benchmark_summary!(prof, timed, dts, T, N, name, algo, bench)
