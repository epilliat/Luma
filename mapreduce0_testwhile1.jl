...
function mapreducekernel(
    ...
    #@cuprintln("$(laneid()), $(typeof(laneid()))")
    lane = Int(laneid())#((tid - 1) % warpsz + 1) #lane
    ...
    if strd <= n
        ...
        gwid = (gid - 1) ÷ warpsz + 1 # global warp index
        gwid > maxnwp && return
    ...
    filtr = (
        (strd <= n && lane == nwp) ||
        (strd > n && (lane == nwp && bid < blocks || lane == cld((n - 1) % threads + 1, warpsz) && bid == blocks))
    )
    @inbounds if filtr
        partial[bid] = val
        threadfence()
...
CUDA.@profile kernel(f, opp, result, Vs, partial, flag, targetflag, glmemlength; shmem=shmem, threads=threads, blocks=blocks)
