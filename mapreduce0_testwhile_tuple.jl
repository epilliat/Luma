...
function mapreducekernel(
    ...
    glmemlength::Int,
) where {F<:Function,O<:Function,Outf}
    n = Int(length(Vs))
    ...
    if strd <= n
        ...
        gwid = (gid - Int(1)) ÷ warpsz + Int(1) # global warp index
        gwid > maxnwp && return
    ...
    filtr = (
        (strd <= n && lane == nwp) ||
        (strd > n && (lane == nwp && bid < blocks || lane == cld((n - Int(1)) % threads + Int(1), warpsz) && bid == blocks))
    )
    @inbounds if filtr
        partial[bid] = val
        threadfence()
